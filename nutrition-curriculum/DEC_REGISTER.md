...
| `DEC-033` | F · Macros | Determine how the protein requirement is distributed across the day's eating occasions | `PROVISIONAL` | C | IMPLEMENTED, consumer wired 2026-09-23 — see docs/mvp-scope/macros-mvp.md |
| `DEC-034` | F · Macros | Determine an appropriate total daily carbohydrate requirement given goal, energy prescription, and tra… | `SHIPPED` | A | Carb g/kg-by-activity table implemented; `very_high` band corrected from 10-12 (pre-event loading, mistakenly presented as routine) to 6-10 g/kg/day per Gate 6; 130 g/day DRI floor and sedentary/light AMDR (45-65% of energy, replacing the g/kg band) added — see `docs/mvp-scope/macros-mvp.md` |
...
| `DEC-060` | K · Food Selection | Determine the translation boundary between a per-occasion nutrient/macro target and a candidate set of… | `SHIPPED` | A | `scoreAllCombos` translation implemented; `matchEveningCombos` extends it with quantity-solving evening recommendations, see docs/mvp-scope/food-selection-mvp.md |
...
| `DEC-066` | L · Meal Construction/Prep | Determine how a set of selected foods and portions is translated into constructed meals | `COVERED` | G | `combos.json`/`combos.ts` already provide curated constructed meals — adequate for MVP; generative construction is a…; portion tiers (Küçük/Normal/Büyük ×⅔/×1/×1⅓, custom multiplier) shipped 2026-09-19 via the Meal Plan picker, see docs/mvp-scope/meal-construction-mvp.md; user-authored saved meals (Yemeklerim, per-user `saved_meals`) extend construction 2026-09-20, see docs/mvp-scope/meal-construction-mvp.md |
| `DEC-067` | L · Meal Construction/Prep | Determine what level of preparation detail (recipe-level, ingredient-list-level, or general guidance o… | `SHIPPED` | A | Ratified Level 1, implemented exactly at that level for Grocery's curated meals; user-authored meals may carry the user's own free-text preparation steps (display-only, not Grocery guidance), scoped 2026-09-20 with the owner's approval — see docs/mvp-scope/meal-construction-mvp.md |
...
| `DEC-069` | L · Meal Construction/Prep | Determine how batch cooking, leftovers, and storage are incorporated into a meal plan when relevant | `SHIPPED` | A | Fully implemented, live-validated; UI re-surfaced in Yemek Planı 2026-09-19 (see docs/mvp-scope/meal-construction-mvp.md); UI hidden since 2026-09-20 pending re-wiring to the renamed Yemekler sheet, see docs/mvp-scope/meal-construction-mvp.md |
| `DEC-070` | L · Meal Construction/Prep | Determine how a meal plan is adjusted when the user deviates from it (skipped meal, substituted food… | `SHIPPED` | A | Already implemented — `removeItem`/`ConfirmDeleteButton` in `MealPlanView.tsx` already let a user delete/adjust a pla…; meal cards, slot/day clear and a 5-step undo added 2026-09-21, see docs/mvp-scope/meal-construction-mvp.md |
