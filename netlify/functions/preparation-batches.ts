...
// src/lib/preparationBatch.ts and nutrition-curriculum/08_APP_TRANSLATION/
// DEC-069_IMPLEMENTATION_PLAN.md. `composition` is written once here and
// never updated by this file — there is deliberately no PATCH/DELETE
// endpoint (§6.3 of the plan: a correction creates a new batch, it never
// edits an existing one). Uses PostgREST anon key for reads and
// service_role key for writes, both gated by owner/invited household
// access — mirrors meal-entries.ts exactly.
//
...
// UUID. This function does not interpret food_id at all; it is validated
// only as a non-empty string, exactly like meal-entries.ts's food_id.
