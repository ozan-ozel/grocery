...
async function handleRead(request: Request): Promise<Response> {
  ...
  const byNameUrl = `${base}/nutrition?select=${SELECT_COLS}&name_tr=in.${inList}`;
  const byAliasUrl = `${base}/nutrition?select=${SELECT_COLS}&aliases=ov.${ovList}`;
...
async function handleWrite(request: Request): Promise<Response> {
  ...
  const res = await fetch(`${restBase(supabaseUrl)}/nutrition?select=${SELECT_COLS}`, {
    method: "POST",
    headers: {
      apikey: serviceKey,
