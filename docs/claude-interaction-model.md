...
| Checkpoints and follow-ups | `session-checkpoint` (a historical record plus its `README.md` index row), `current-state-update` (updates `docs/CURRENT_STATE.md`), `session-log` (the vault) — all in `.claude/skills/` | as written; they keep the two systems apart: `CURRENT_STATE.md` is the current-state guide, `docs/session-checkpoints/` the historical record |
