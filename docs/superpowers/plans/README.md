...
| [2026-09-02-nutrition-personal-plan-ux-polish-summary.md](2026-09-02-nutrition-personal-plan-ux-polish-summary.md) | 2026-09-02 | `SHIPPED` | Own text says "merged to master, pushed"; verified in code — `AppBootSkeleton`/`useDetailsTransition`/`.gradient-edge-flow` all present in `App.tsx`/`index.css`/`useDetailsTransition.ts` |
...
| [2026-09-04-sport-nutrition-integration-handoff.md](2026-09-04-sport-nutrition-integration-handoff.md) | 2026-09-04 | `SHIPPED` | A continuation note, not a formal plan — of its 4 tracked items, A (protein) and B (carb g/kg model) are both shipped (`mealPersonalization.ts`'s `very_high: { min: 6, max: 10 }` matches the note's target table and cites DEC-034/Gate 6; `feature/personal-plan-carb-target-model` branch no longer exists, consistent with merge). C (PAL split) and D (sodium/fluid target) were never picked up — no trace in code or `DEC_REGISTER.md` — and remain open future work outside this note's own scope |
...
| [2026-09-12-meal-plan-header-cleanup.md](2026-09-12-meal-plan-header-cleanup.md) | 2026-09-12 | `SHIPPED` | Outcome achieved, but by a broader later redesign, not this plan's own diff: `AppHeader.tsx` now does `if (section !== "alisveris") return null` (all three non-shopping sections, not just `"yemek"` as this plan scoped) |
| [2026-09-12-meal-plan-shopping-cleanup.md](2026-09-12-meal-plan-shopping-cleanup.md) | 2026-09-12 | `SHIPPED` | Verified in `MealPlanView.tsx`: `toggleDayShoppingList`/`dayAlreadyOnList` present (Task 2); no `BatchPlanner` reference anywhere in `src/` (Task 1) |
...
| [2026-09-23-meal-plan-protein-target-consumer.md](2026-09-23-meal-plan-protein-target-consumer.md) | 2026-09-23 | `SHIPPED` | MealContainer.tsx header shows per-slot protein target band; DEC-033 spec in nutrition-curriculum/09_HANDOFF_SPECS/ |
