  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        quiet: "text-muted-foreground hover:text-foreground active:text-foreground",
        destructive: "text-signal hover:bg-signal/10 active:bg-signal/10",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    // Radix's Slot and the intrinsic "button" have ref types that don't
    // reconcile under preact/compat. The ref passes through fine at runtime.
